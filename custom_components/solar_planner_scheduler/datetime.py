"""Datetime platform — the computed (or user-forced) next start time for a device.
...
from .const import ATTR_COVERAGE_PCT, ATTR_END, ATTR_LOCKED, ATTR_POWER_W, ATTR_PROFILE, CONF_DEVICES, CONF_NAME, DOMAIN
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    async_add_entities(StartTimeDateTime(coordinator, entry, name) for name in device_names)


class StartTimeDateTime(CoordinatorEntity[SolarPlannerSchedulerCoordinator], DateTimeEntity):
    """The device's next start time — computed automatically, or forced by setting a value."""
    ...
    def __init__(self, coordinator: SolarPlannerSchedulerCoordinator, entry: ConfigEntry, device_name: str) -> None:
        ...
        self._attr_unique_id = f"{entry.entry_id}_{device_name}_start"
        self._attr_name = f"{device_name} start"

    @property
    def native_value(self) -> datetime | None:
        schedule = self.coordinator.data.get(self._device_name) if self.coordinator.data else None
    ...
    async def async_set_value(self, value: datetime) -> None:
        await self.coordinator.async_set_forced_start(self._device_name, value)
